...
class EventController extends Controller
{
    ...
    public function show($id_empresa)
    {
        ...
        $list_data_of_events_citas = $this->model
            ->with(['trabajador'])
            ->whereNotNull('trabajador_id')
            ->where('start_date', '>=', $start)
            ->where('end_date', '<=', $end)
            ->where('user_id', $user_id)
            ->orWhere('share_with', 'cita')
            ->get();

        foreach ($list_data_of_events_citas as $data) {
            $data->cycle = 0;
            $result[] = $this->_make_leave_event($data);
        }
    ...
    private function _make_leave_event($data) {
        return array(
            "title" => "Cita de " .$data->trabajador->nombre . " " .$data->trabajador->apellido. " - " . $data->title,
            "icon" => "fa fa-sign-out",
