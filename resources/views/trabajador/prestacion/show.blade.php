...
@section('titulo',  'Gestion de pedidos de '. $trabajador->nombre. ' '.$trabajador->apellido)
...
@section('main-content')
    ...
    <div class="modal fade" id="newCotizacion" tabindex="-1" role="dialog" data-backdrop="static">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                @include('trabajador.prestacion.cotizacion_form')
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="newPresupesto" tabindex="-1" role="dialog" data-backdrop="static">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                @include('trabajador.prestacion.presupuesto_form')
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="box box-primary">
                ...
                <!-- /.box-body -->
                <div class="box-body">
                    <p>Pedido de prestación #{{ $prestacion_pedido->id }}</p>
                    <p>Relativo a: {{ $prestacion_pedido->trabajador->nombre }} {{ $prestacion_pedido->trabajador->apellido }}</p>
                    ...
                    <table class="table table-striped table-bordered" id="cotizaciones">
                        ...
                        @foreach  ($prestacion_cotizaciones as $key => $prestacion_cotizacion)
                            <tr id="cotizacion_{{ $prestacion_cotizacion->id }}">
                                ...
                                <td>
                                    <a href="{{ route('trabajador.documentacion.generate',['empresa_id' => $empresa->id,  'trabajador_id' =>  $prestacion_pedido->trabajador->id, 'filename' => $prestacion_cotizacion->cotizacion_url, 'documento_id' => $prestacion_cotizacion->id, 'type' => 'cotizacion'] ) }}">{{ $prestacion_cotizacion->cotizacion_url }}</a>
                                </td>
                                ...
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        ...
                                        <a class="btn btn-warning" title="Edit" href="#editCotizacion" data-toggle="modal" data-href="{{route('trabajador.cotizacion.pedido.edit', ['id' => $prestacion_cotizacion->id, 'id_empresa' => $empresa->id , 'pedido_id' => $prestacion_pedido->id])}}"><i title="Editar cotizacion" class="fa fa-pencil"></i></a>

                                        {!! method_field('DELETE') !!}
                                        @csrf
                                        <input type="hidden" name="empresa_id" value="{{ $empresa->id }}">
                                        <button type="submit" class="btn btn-danger delete-confirm" data-id="{{ $prestacion_cotizacion->id }}" data-href="{{ route('trabajador.cotizacion.pedido.destroy', ['id' => $prestacion_cotizacion->id]) }}">
                                            <i class="fa fa-trash"></i>
                                        </button>


                                        <button type="submit" class="btn btn-success confirm-cotizacion" data-id="{{ $empresa->id }}" data-href="{{ route('trabajador.cotizacion.pedido.update', ['id' => $prestacion_cotizacion->id, 'prestacion_pedido_id' =>  $prestacion_pedido->id ]) }}">
                    ...
                    <table class="table table-striped table-bordered" id="presupuesto_list">
                        ...
                        @foreach  ($prestacion_presupuestos as $key => $prestacion_presupuesto)
                            <tr id="presupuesto_{{ $prestacion_presupuesto->id }}">
                                ...
                                <td>
                                    <a href="{{ route('trabajador.documentacion.generate',['empresa_id' => $empresa->id,  'trabajador_id' =>  $prestacion_pedido->trabajador->id, 'filename' => $prestacion_presupuesto->presupuesto_url, 'documento_id' => $prestacion_presupuesto->id, 'type' => 'presupuesto'] ) }}">{{ $prestacion_presupuesto->presupuesto_url }}</a>
                                </td>
                                ...
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            ...
                                            @csrf
                                            <input type="hidden" name="empresa_id" value="{{ $empresa->id }}">
                                            <button type="submit" class="btn btn-danger delete-confirm-presupuesto" data-id="{{ $prestacion_presupuesto->id }}" data-href="{{ route('trabajador.presupuesto.pedido.destroy', ['id' => $prestacion_presupuesto->id]) }}">
                                                <i class="fa fa-trash"></i>
                                            </button>


                                            <button type="submit" class="btn btn-success confirm-presupuesto" data-id="{{ $empresa->id }}" data-href="{{ route('trabajador.presupuesto.pedido.update', ['id' => $prestacion_presupuesto->id, 'prestacion_pedido_id' =>  $prestacion_pedido->id ]) }}">
                                                <i class="fa fa-check"></i>
                                            </button>
...
<div class="row">
  <div class="col-md-6">
      @isset($orden_servicio)
          {{ Form::model($orden_servicio, ['route' => ['trabajador.orden.pedido.update', $orden_servicio->id, $empresa->id], 'role' => 'form', 'method' => 'PATCH', 'id' => 'frm', 'enctype' => 'multipart/form-data'])
          }}
      @endisset

      @empty ($orden_servicio)
           {{ Form::open(['route' => 'trabajador.orden.pedido.store', 'role' => 'form', 'method' => 'post', 'enctype' => 'multipart/form-data']) }}
      @endempty
      ...
      <div class="box">
          ...
          <div class="box-body table-responsive">
              ...
              <div class="col-md-12">
                  {{ Form::label('orden_servicio', "Orden de servicio") }}
                  {{ Form::textarea('orden_servicio',  isset($orden_servicio) ? $orden_servicio->orden_servicio : null, ['class' => 'form-control box-size', 'placeholder' => "Orden de servicio"]) }}
                  @if ($errors->has('orden_servicio'))
                      <span class="help-block">
                            <strong>{{ $errors->first('orden_servicio') }}</strong>
                        </span>
                  @else
                      <span class="help-block" id="orden_servicio"></span>
                  @endif
              </div>
  ...
  <div class="col-md-6">
      {{ Form::model($prestacion_pedido, ['route' => ['trabajador.reporte.pedido.update', $prestacion_pedido->id, $empresa->id], 'role' => 'form', 'method' => 'PATCH', 'id' => 'frm', 'enctype' => 'multipart/form-data'])
      }}
      <div class="box">
          <div class="box-header">
              <h3 class="box-title">
          ...
          <div class="box-body table-responsive">
              ...
              <div class="col-md-12">
                  ...
                  {{ Form::textarea('reporte',  isset($prestacion_pedido) ? $prestacion_pedido->reporte : null, ['class' => 'form-control box-size', 'placeholder' => "Reporte"]) }}
              ...
              <div class="col-md-12">
                  ...
                  {{ Form::text('feedback',  isset($prestacion_pedido) ? $prestacion_pedido->feedback : null, ['class' => 'form-control box-size', 'placeholder' => "Feedback"]) }}
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#orden_servicio_url').fileinput({
                ...
                initialPreview: ["{{ asset('storage/empresas/' . $empresa->id . '/trabajadores/' . $prestacion_pedido->trabajador->id . '/orden_servicio/'. $orden_servicio->id. '/'. $orden_servicio->orden_servicio_url) }}"],
            ...
            $('#reporte_url').fileinput({
                ...
                previewTemplates: 'file-preview-pdf',
                initialPreviewFileType: 'pdf',
                initialPreview: ["{{ asset('storage/empresas/' . $empresa->id . '/' . $prestacion_pedido->reporte_url) }}"],
                initialPreviewAsData: true,
                initialPreviewDownloadUrl: "",
                <?php } ?>
                maxFileCount: 5,
