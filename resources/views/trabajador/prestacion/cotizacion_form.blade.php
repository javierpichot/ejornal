@isset($prestacion_cotizacion)
    <script type="text/javascript">
        $(function () {
            $('#cotizacion_url').fileinput({
                ...
                initialPreview: ["{{ asset('storage/empresas/' . $empresa->id.'/trabajadores/'. $prestacion_pedido->trabajador->id . '/cotizacion/' . $prestacion_cotizacion->id . '/' . $prestacion_cotizacion->cotizacion_url) }}"],
                initialPreviewAsData: true,
                initialPreviewDownloadUrl: "{{ route('trabajador.documentacion.generate',['empresa_id' => $empresa->id,  'trabajador_id' =>  $prestacion_pedido->trabajador->id, 'filename' => $prestacion_cotizacion->cotizacion_url, 'documento_id' => $prestacion_cotizacion->id, 'type' => 'cotizacion'] ) }}",
                <?php } ?>
                maxFileCount: 5,
                theme: 'fa',
                language: 'es',
    ...
    {{ Form::model($prestacion_cotizacion, ['route' => ['trabajador.cotizacion.pedido.update',$prestacion_cotizacion->id , $prestacion_pedido->id], 'role' => 'form', 'method' => 'PATCH', 'id' => 'frm'])
    }}
@endisset

@empty ($cotizacion)
    {{ Form::open(['route' => 'trabajador.cotizacion.pedido.store', 'role' => 'form', 'method' => 'post', 'id' => 'frm']) }}
@endempty
