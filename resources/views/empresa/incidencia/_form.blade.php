...
@isset($incidencia)
               ...
               $('#fecha_incidencia').datetimepicker({
                  format: 'YYYY-MM-DD HH:mm'              });
                   ...
                   $('#fotos_escenario').fileinput({
                      ...
                      initialPreview: ["{{ asset('storage/empresas/'. $empresa->id  .'/seh/'.  $incidencia->id .'/'.  $incidencia->fotos_escenario) }}"],
                      initialPreviewAsData: true,
                      initialPreviewDownloadUrl: "{{ route('empresa.documentacion.generate',['empresa_id' =>  $empresa->id, 'filename' => $incidencia->fotos_escenario,  'documento_id' => $incidencia->id, 'type' => 'seh'] ) }}",
                      <?php } ?>
                      maxFileCount: 5,
                      theme: 'fa',
                      language: 'es',
                  ...
                  $('#declaracion_accidente').fileinput({
                      ...
                      previewTemplates: 'file-preview-pdf',
                      initialPreviewFileType: 'pdf',
                      initialPreview: ["{{ asset('storage/empresas/'. $empresa->id  .'/seh/'.  $incidencia->id .'/'.  $incidencia->fotos_accidente) }}"],
                      initialPreviewAsData: true,
                      initialPreviewDownloadUrl: "{{ route('empresa.documentacion.generate',['empresa_id' =>  $empresa->id, 'filename' => $incidencia->fotos_accidente,  'documento_id' => $incidencia->id, 'type' => 'seh'] ) }}",
                      <?php } ?>
                      maxFileCount: 5,
                      theme: 'fa',
                      language: 'es',
                  ...
                  $('#fotos_lesiones').fileinput({
                      ...
                      previewTemplates: 'file-preview-pdf',
                      initialPreviewFileType: 'pdf',
                      initialPreview: ["{{ asset('storage/empresas/'. $empresa->id .'/seh/'.  $incidencia->id .'/'.  $incidencia->fotos_lesion) }}"],
                      initialPreviewAsData: true,
                      initialPreviewDownloadUrl: "{{ route('empresa.documentacion.generate',['empresa_id' =>  $empresa->id, 'filename' => $incidencia->fotos_lesion,  'documento_id' => $incidencia->id, 'type' => 'seh'] ) }}",
                      <?php } ?>
                      maxFileCount: 5,
                      theme: 'fa',
                      language: 'es',
    ...
    {{ Form::model($incidencia, ['route' => ['empresa.incidencias.update',$incidencia->id,$empresa->id ], 'enctype' => 'multipart/form-data', 'role' => 'form', 'method' => 'PATCH', 'id' => 'frm'])
    }}
    <input type="hidden" name="empresa_id" value="{{ $incidencia->empresa_id }}">
@endisset

@empty ($incidencia)
    {{ Form::open(['route' => 'empresa.incidencia.store', 'role' => 'form', 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'frm']) }}
    <input type="hidden" name="empresa_id" value="{{ $empresa->id }}">
    <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
@endempty
<div class="modal-header">
    <h5 class="modal-title">{{isset($incidencia)?'Editar':'Crear nueva'}} incidencia relativo a {{ $empresa->nombre }}</h5>
...
<div class="modal-body">
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-4 mb-3">
            ...
            {!! Form::select('forma_accidente_id', $forma_accidentes->pluck('nombre', 'id'), isset($incidencia) ? $incidencia->forma_accidente_id : null, ['placeholder' => 'Consultas', 'class' => 'form-control box-size']) !!}
        ...
        <div class="form-group col-md-4 mb-3">
            ...
            {!! Form::select('tipo_lesion_id', $tipo_lesions->pluck('nombre', 'id'), isset($incidencia) ? $incidencia->tipo_lesion_id : null, ['placeholder' => 'Consultas', 'class' => 'form-control box-size']) !!}
        ...
        <div class="form-group col-md-4 mb-3">
            ...
            {!! Form::select('ubicacion_lesion_id', $ubicacion_lesions->pluck('nombre', 'id'), isset($incidencia) ? $incidencia->ubicacion_lesion_id : null, ['placeholder' => 'Consultas', 'class' => 'form-control box-size']) !!}
