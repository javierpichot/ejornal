@isset($consulta)
    ...
    {{ Form::model($consulta, ['route' => ['trabajador.consulta.update',$consulta->id , $empresa->id], 'role' => 'form', 'method' => 'PATCH', 'id' => 'frm'])
    }}
...
@empty ($consulta)
    {{ Form::open(['route' => 'trabajador.consulta.store', 'role' => 'form', 'method' => 'post', 'id' => 'frm']) }}
...
<div class="modal-body">
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            @forelse($ausentismos as $key => $row)
                @isset($consulta->ausentismo_id)
                    <select class="form-control" name="ausentismo_id">
                        <option @if ($consulta->ausentismo_id == $row->id)
                                selected
                                @endif value="{{ $row->id }}">{{ $row->motivo }} {{ $row->fecha_ausente }}
                        </option>
                    </select>
                @endisset

            @empty ($consulta->ausentismo_id)
                <select class="form-control" name="ausentismo_id">
                    <option value="{{ $row->id }}">{{  $row->motivo }} {{ $row->fecha_ausente }}
                    </option>
                </select>
            @endempty
            ...
            @empty
                <select class="form-control" name="ausentismo_id">
                    <option  value="">No posee ausencias</option>
                </select>
            @endforelse
    ...
    <div id="capa_medica" style="display: {{ isset($consulta) ? ($consulta->consulta_tipo_id == 1) ? 'block' : 'none' : 'none' }}" >
        <div class="row">
            <div class="col-xs-6">
                ...
                {{ Form::textarea('entrevista',  isset($consulta) ? $consulta->entrevista : null, ['class' => 'form-control box-size', 'placeholder' => "Anamnesis", 'rows' => 3]) }}
            ...
            <div class="col-xs-6">
                ...
                {{ Form::textarea('examen_fisico',  isset($consulta) ? $consulta->examen_fisico : null, ['class' => 'form-control box-size', 'placeholder' => "Examen fisico", 'rows' => 3]) }}
            ...
            <div class="col-xs-6">
                {{ Form::label('nueva_cita', "Nueva cita") }}
                 <div id="fecha_cita" class="input-group date">
                     ...
                     {{ Form::text('nueva_cita',  isset($consulta) ? $consulta->nueva_cita : null, ['class' => 'form-control box-size', 'placeholder' => "Nueva cita", 'id' => 'nueva_cita']) }}
    ...
    <div id="capa_enfermeria" style="display: {{ isset($consulta) ? ($consulta->consulta_tipo_id == 2) ? 'block' : 'none' : 'none' }}">
        <div class="row">
            <div class="col-xs-12">
                ...
                {{ Form::textarea('descripcion',  isset($consulta) ? $consulta->descripcion : null, ['class' => 'form-control box-size', 'placeholder' => "Descripcion", 'rows' => 3]) }}
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                {!! Form::select('consulta_reposo_id', $consulta_reposo->pluck('nombre', 'id'), isset($consulta) ? $consulta->consulta_reposo_id : null, ['placeholder' => 'Amerita salida', 'class' => 'form-control box-size']) !!}
